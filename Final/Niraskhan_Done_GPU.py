# Environment: D:\CUDA_Experiments\venv\Scripts\Activate.ps1
...
try:
    ...
    print(f"GPU libraries not found: {e}")
    GPU_AVAILABLE = False
    ...
    try:
        ...
        return {
            "detection": {"min_detection_confidence": 0.5, "min_tracking_confidence": 0.5, 
                         "face_recognition_tolerance": 0.58, "re_detect_interval": 8},  # ✅ OPTIMIZED: 0.58 tolerance for better detection, 8 frame interval for performance
            "alert": {"default_interval_seconds": 10, "alert_cooldown_seconds": 2.5},
            "performance": {"gui_refresh_ms": 30, "pose_buffer_size": 12, "frame_skip_interval": 3, "enable_frame_skipping": True, "min_buffer_for_classification": 5},
...
class InsightFace_GPU:
    def __init__(self):
        self.app = None
        if GPU_AVAILABLE:
            try:
                # Initialize InsightFace with GPU providers
                # Assuming models are in the default insightface path or handled by the library
                # User specified: det_10g.onnx + w600k_r50.onnx
                # We can specify these if we construct the app manually, or let FaceAnalysis handle it if installed
                # For strict adherence to "det_10g.onnx + w600k_r50.onnx", we might need to configure FaceAnalysis
                # 'buffalo_l' uses w600k_r50. 'antelopev2' uses others.
                # We'll try to load 'buffalo_l' which typically includes w600k_r50.
                # det_10g is usually 'scrfd_10g_bnkps'.
                
                providers = ['CUDAExecutionProvider', 'CPUExecutionProvider']
                self.app = FaceAnalysis(name='buffalo_l', providers=providers)
                self.app.prepare(ctx_id=0, det_size=(640, 640))
                print("✅ InsightFace GPU initialized successfully")
            except Exception as e:
    ...
    def detect_faces(self, img):
        if self.app is None:
            # Fallback to face_recognition (CPU) if GPU init failed
            # But we should try to use the new logic. 
            # If app is None, we return empty or handle gracefully.
            return []
    ...
    def get_face_locations(self, img):
        # Returns list of (top, right, bottom, left) to match face_recognition API
    ...
    def get_encodings(self, img, known_face_locations=None):
        # InsightFace computes embeddings during detection
        # If known_face_locations is provided, we might need to crop and re-infer, 
        # but InsightFace works best on full image.
        # For compatibility, if we already have faces from detect_faces, we return their embeddings.
        
        faces = self.detect_faces(img)
        encodings = [face.embedding for face in faces]
        return encodings

    def compute_distance(self, known_encodings, unknown_encoding):
        # Compute cosine distance (or Euclidean). InsightFace embeddings are usually normalized.
        # face_recognition uses Euclidean distance.
        # We'll use Euclidean to match the threshold scale (0.6 etc).
        ...
        # Euclidean distance
        diff = known_encodings - unknown_encoding
        dist = np.linalg.norm(diff, axis=1)
        return dist

class YOLO_TensorRT:
    def __init__(self, engine_path="yolov8x-pose-p6.engine"):
        self.logger = trt.Logger(trt.Logger.WARNING)
        self.runtime = trt.Runtime(self.logger)
        self.engine = self.load_engine(engine_path)
        self.context = self.engine.create_execution_context() if self.engine else None
        self.inputs, self.outputs, self.bindings, self.stream = self.allocate_buffers() if self.engine else ([], [], [], None)
    ...
    def infer(self, frame):
        ...
        # Assuming YOLOv8 input size 640x640 or 1280x1280 (p6)
        # Need to check engine input shape
        input_shape = self.engine.get_binding_shape(0) # usually [1, 3, h, w]
        ...
        # This depends on YOLO output format (usually [1, 84, 8400] for v8)
        # For now, returning raw output or simplified list
        return self.outputs[0]['host']

    def detect_persons(self, frame, conf_thres=0.5, iou_thres=0.45):
        if not self.context:
            return []
        
        # Preprocess
        input_shape = self.engine.get_binding_shape(0)
        h_input, w_input = input_shape[2], input_shape[3]
        h_orig, w_orig = frame.shape[:2]
        
        img = cv2.resize(frame, (w_input, h_input))
        img = cv2.cvtColor(img, cv2.COLOR_BGR2RGB)
        img = img.transpose((2, 0, 1)).astype(np.float32)
        img /= 255.0
        img = np.expand_dims(img, axis=0)
        ...
        # Handle dynamic shapes if needed, but usually fixed
        # Assuming output is flattened
        try:
        ...
        # Filter by confidence for class 0 (person)
        # box format: cx, cy, w, h
        
        for row in output:
            # YOLOv8 output is [cx, cy, w, h, class0, class1, ...]
            score = row[4] # Class 0 score (assuming COCO where 0 is person)
            
            if score > conf_thres:
                cx, cy, w, h = row[0], row[1], row[2], row[3]
                
                # Scale back to original image
                x1 = int((cx - w/2) * w_orig / w_input)
                y1 = int((cy - h/2) * h_orig / h_input)
                x2 = int((cx + w/2) * w_orig / w_input)
                y2 = int((cy + h/2) * h_orig / h_input)
        ...
        if len(indices) > 0:
            for i in indices.flatten():
                x, y, w, h = boxes[i]
                results.append((x, y, x+w, y+h)) # x1, y1, x2, y2
...
# ✅ THREADED IP CAMERA FRAME GRABBER - For smooth RTSP streaming
class ThreadedIPCamera:
    ...
    def start(self):
        ...
        # ✅ UBUNTU COMPATIBILITY: Use V4L2 if on Linux and using local device
        if platform.system() == "Linux" and isinstance(self.rtsp_url, int):
             self.cap = cv2.VideoCapture(self.rtsp_url, cv2.CAP_V4L2)
        else:
        ...
        # ✅ CRITICAL: RTSP stream optimization settings
        # Reduce buffer size to get latest frame (not buffered old frames)
        self.cap.set(cv2.CAP_PROP_BUFFERSIZE, 1)
        # Set lower resolution for faster streaming (can be upscaled if needed)
    ...
    def _update(self):
        ...
        while self.running:
            try:
                ...
                # ✅ CRITICAL: Grab multiple frames to clear buffer and get latest
                # This prevents lag from buffered old frames
                for _ in range(2):  # Discard 2 frames to get fresh one
                    self.cap.grab()
                ...
                if grabbed and frame is not None:
                    ...
                    # Frame grab failed
                    if time.time() - self.last_frame_time > 5:  # 5 second timeout
    ...
    def _reconnect(self):
        ...
        if self.reconnect_attempts >= self.max_reconnect_attempts:
            logger.error("IP Camera: Max reconnection attempts reached")
            self.running = False
            return
        
        self.reconnect_attempts += 1
        logger.warning(f"IP Camera: Reconnecting... (attempt {self.reconnect_attempts})")
...
def detect_faces_multiscale_distance(rgb_frame, pro_mode=False):
    ...
    """
    # ✅ GPU ACCELERATION: Use InsightFace if available
    # We need access to the app instance. Since this is a standalone function, 
    # we might need to pass the app instance or make it global.
    # However, the user asked to preserve structure.
    # I'll check if I can access the app instance from here.
    # It seems this function is outside the class.
    # I'll assume the caller will handle it or I should modify the caller.
    # But wait, this function is called by the App class.
    # I'll modify the function to accept an optional detector argument or check a global.
    # Or better, I'll modify the App class to call a method instead of this function, 
    # OR I'll modify this function to use a global instance if I created one.
    # I didn't create a global instance. I created it inside App.
    
    # Strategy: I will modify the App class to use `self.insightface.get_face_locations(frame)` 
    # instead of calling this function, OR I will modify this function to use `face_recognition` as fallback
    # and the App class will call its own method.
    
    # Actually, `detect_faces_multiscale_distance` is a standalone function.
    # I should probably leave it as is (CPU fallback) and modify the App class to use InsightFace directly
    # where it calls this function.
    
    # Let's see where it is called.
    # It is called in `process_tracking_frame_optimized`? No, that used `face_recognition.face_locations` directly.
    # I'll search for usages of `detect_faces_multiscale_distance`.
...
class PoseApp:
    def __init__(self, window_title=" निराक्षण - Niraakshan"):
        ...
        self.is_ip_camera = False  # ✅ Track if using IP camera (for threaded grabber)
        self.threaded_cam = None  # ✅ Threaded IP camera instance for smooth RTSP streaming
        
        # ✅ STABILITY: Tracking identity confirmation (prevents rapid switching)
        self.identity_confirmation_frames = 5  # Require 5 consecutive frames to confirm identity switch
        self.guard_fugitive_margin = 0.12  # Minimum distance margin to distinguish guard from fugitive
    ...
    def _initialize_model_pipeline(self):
        ...
        try:
            # ✅ GPU ACCELERATION: Initialize InsightFace and YOLO
            if GPU_AVAILABLE:
                logger.info("Initializing GPU models...")
                self.insightface = InsightFace_GPU()
                self.yolo = YOLO_TensorRT("yolov8x-pose-p6.engine")
                if self.insightface.app is not None:
                    logger.info("✅ InsightFace GPU initialized")
                if self.yolo.context is not None:
                    logger.info("✅ YOLO TensorRT initialized")
            else:
                self.insightface = None
                self.yolo = None
                logger.warning("GPU not available - falling back to CPU/Legacy models")
    ...
    def get_adaptive_face_detection_params(self, frame):
        ...
        # ✅ BALANCED VALUES: Fast detection but NO ghost/false positives
        if brightness < 50:  # Very dark
            face_tolerance = 0.52  # STRICTER for dark
            ...
            face_tolerance = 0.55  # BALANCED for bright
    ...
    def open_language_converter(self):
        ...
        try:
            ...
            # Linux/Ubuntu Fix: Ensure window is shown and on top
            dialog.lift()
            dialog.focus_force()
            dialog.grab_set()
    ...
    def remove_guard_dialog(self):
        ...
        ctk.CTkLabel(dialog, text="Select guard to remove:", 
                font=('Roboto', 14, 'bold')).pack(pady=10)
        ...
        ctk.CTkButton(btn_frame, text="Remove", command=on_remove, fg_color="#e74c3c", 
                 font=('Roboto', 12, 'bold'), width=100).pack(side="left", padx=5)
        ctk.CTkButton(btn_frame, text="Cancel", command=dialog.destroy, fg_color="#7f8c8d", 
                 font=('Roboto', 12, 'bold'), width=100).pack(side="left", padx=5)
    ...
    def upload_guard_image(self):
        """Upload an existing image for guard onboarding with Interactive ROI Selection"""
        # if not self.is_running: return # Allow uploading even if camera not running
        ...
        try:
            ...
            # --- INTERACTIVE SELECTION LOGIC (from app.py) ---
            messagebox.showinfo("Instructions", "1. Draw a box around the Target's FACE.\n2. Press ENTER or SPACE to confirm.\n3. Press c to cancel.")
            
            # Create a window for ROI selection
            cv2.namedWindow("Select Target Face", cv2.WINDOW_NORMAL)
            cv2.setWindowProperty("Select Target Face", cv2.WND_PROP_TOPMOST, 1)
            
            r = cv2.selectROI("Select Target Face", img, showCrosshair=True)
            cv2.destroyWindow("Select Target Face")
            
            x, y, w, h = [int(i) for i in r]
            if w == 0 or h == 0:
                messagebox.showwarning("Cancelled", "No selection made.")
                return

            # Try to detect face in the crop first
            face_crop = img[y:y+h, x:x+w]
            
            best_face_crop = None
            
            if self.insightface and self.insightface.app is not None:
                # Use InsightFace
                results = self.insightface.app.get(face_crop)
                
                if len(results) == 0:
                    logger.warning("No face in crop, checking full image context...")
                    all_faces = self.insightface.app.get(img)
                    ...
                    for face in all_faces:
                        ...
                        # Check if face center is reasonably close to ROI center
                        if dist < max(w, h) * 1.5 and dist < min_dist:
                            min_dist = dist
                            best_face = face
                    
                    if best_face:
                        # Found the face in full image corresponding to ROI
                        # Crop this face with some padding
                        bbox = best_face.bbox.astype(int)
                        bx1, by1, bx2, by2 = bbox
                        # Add padding
                        pad_w = int((bx2 - bx1) * 0.2)
                        pad_h = int((by2 - by1) * 0.2)
                        cx1 = max(0, bx1 - pad_w)
                        cy1 = max(0, by1 - pad_h)
                        cx2 = min(img.shape[1], bx2 + pad_w)
                        cy2 = min(img.shape[0], by2 + pad_h)
                        best_face_crop = img[cy1:cy2, cx1:cx2]
                    else:
                        # No matching face found
                        pass
                else:
                    # Face found in crop
                    # Get the largest face in crop
                    target_face = max(results, key=lambda x: (x.bbox[2]-x.bbox[0]) * (x.bbox[3]-x.bbox[1]))
                    best_face_crop = face_crop

            else:
                # Fallback to face_recognition (CPU)
                face_locs = face_recognition.face_locations(face_crop)
                if not face_locs:
                    # Check full image
                    all_face_locs = face_recognition.face_locations(img)
                    roi_center = (x + w/2, y + h/2)
                    best_loc = None
                    min_dist = float('inf')
                    
                    for (top, right, bottom, left) in all_face_locs:
                        fc = ((left+right)/2, (top+bottom)/2)
                        dist = ((fc[0]-roi_center[0])**2 + (fc[1]-roi_center[1])**2)**0.5
                        if dist < max(w, h) * 1.5 and dist < min_dist:
                            min_dist = dist
                    ...
                    if best_loc:
                        ...
                        h_face = bottom - top
                        w_face = right - left
                        pad_w = int(w_face * 0.2)
                        pad_h = int(h_face * 0.2)
                        cx1 = max(0, left - pad_w)
                        cy1 = max(0, top - pad_h)
                        cx2 = min(img.shape[1], right + pad_w)
                        cy2 = min(img.shape[0], bottom + pad_h)
                        best_face_crop = img[cy1:cy2, cx1:cx2]
                else:
                    best_face_crop = face_crop

            if best_face_crop is None:
                messagebox.showerror("Error", "No face found in selection. Please try again with a clearer image or selection.")
                return

            # Save the best face crop
            cv2.imwrite(target_path, best_face_crop)
            
            # Also copy to root for backward compatibility
            cv2.imwrite(f"target_{safe_name}_face.jpg", best_face_crop)
            
            self.load_targets()
            messagebox.showinfo("Success", f"Guard '{name}' added successfully!\nTarget ID Secured.")
            logger.warning(f"Guard added via interactive selection: {name}")
            ...
            if filename:
                try:
                    ...
                    for angle, img_path in angle_images.items():
                        try:
                            if self.insightface and self.insightface.app is not None:
                                # InsightFace uses cv2 (BGR)
                                angle_image = cv2.imread(img_path)
                                if angle_image is None:
                                    logger.warning(f"Failed to load image: {img_path}")
                                    continue
                                angle_encodings = self.insightface.get_encodings(angle_image)
                            else:
                    ...
                    if not multi_angle_encodings:
                        if self.insightface and self.insightface.app is not None:
                            target_image_file = cv2.imread(filename)
                            if target_image_file is not None:
                                encodings = self.insightface.get_encodings(target_image_file)
                                if encodings:
                    ...
                    if multi_angle_encodings and len(multi_angle_encodings) > 0:
                        self.targets_status[name] = {
                            ...
                            "tracker_drift_counter": 0,  # ✅ DRIFT DETECTION: Count frames where tracker may be following unknown
                            "needs_drift_verification": False,  # ✅ DRIFT DETECTION: Flag to check tracker after face detection
                            "ghost_no_pose_frames": 0,  # ✅ GHOST DETECTION: Count frames with face but no pose
                        }
                        count += 1
    ...
    def open_target_selection_dialog(self):
        ...
        for target in targets:
            ...
            # Image
            filename = self.target_map.get(target)
            if filename and os.path.exists(filename):
                try:
                    ...
                    if img is not None:
                        ...
                        lbl = ctk.CTkLabel(row_frame, image=photo, text="")
                        lbl.pack(side="left", padx=5)
                        # Keep reference
                        if not hasattr(self, 'dialog_photos'): self.dialog_photos = []
                        self.dialog_photos.append(photo)
                except:
                    pass
            
            # Name
            ctk.CTkLabel(row_frame, text=target, font=("Segoe UI", 11)).pack(side="left", padx=5)
    ...
    def set_alert_interval(self):
        ...
        # Create input fields
        ctk.CTkLabel(dialog, text="Alert Interval (Hours : Minutes)", font=("Helvetica", 12, "bold")).pack(pady=10)
        
        frame = ctk.CTkFrame(dialog)
        frame.pack(pady=10)
        
        ctk.CTkLabel(frame, text="Hours:", width=60).grid(row=0, column=0, padx=5)
        hours_var = ctk.StringVar(value=str(hours))
        hours_entry = ctk.CTkEntry(frame, textvariable=hours_var, width=60)
        hours_entry.grid(row=0, column=1, padx=5)
        
        ctk.CTkLabel(frame, text="Minutes:", width=60).grid(row=1, column=0, padx=5)
        minutes_var = ctk.StringVar(value=str(minutes))
        minutes_entry = ctk.CTkEntry(frame, textvariable=minutes_var, width=60)
        ...
        btn_frame = ctk.CTkFrame(dialog, fg_color="transparent")
        btn_frame.pack(pady=10)
        ctk.CTkButton(btn_frame, text="OK", command=on_ok, fg_color="#27ae60", width=80).pack(side="left", padx=5)
        ctk.CTkButton(btn_frame, text="Cancel", command=dialog.destroy, fg_color="#e74c3c", width=80).pack(side="left", padx=5)
    ...
    def switch_camera_dialog(self):
        ...
        ctk.CTkLabel(dialog, text="Select Camera:", font=('Helvetica', 14, 'bold')).pack(pady=15)
        
        # Variable to track camera type (0 = USB camera, 1 = IP camera)
        camera_type_var = tk.IntVar(value=0)
        
        # === USB CAMERAS SECTION ===
        usb_frame = ctk.CTkFrame(dialog)
        usb_frame.pack(pady=5, padx=20, fill="x")
        
        ctk.CTkRadioButton(usb_frame, text="USB Camera", variable=camera_type_var,
                      value=0, font=('Helvetica', 12, 'bold')).pack(anchor="w", padx=10, pady=5)
        
        selected_camera = tk.IntVar(value=self.camera_index if hasattr(self, 'camera_index') and isinstance(self.camera_index, int) else available_cameras[0])
        
        # Frame for radio buttons
        radio_frame = ctk.CTkFrame(usb_frame, fg_color="transparent")
        ...
        for idx in available_cameras:
            ...
            ctk.CTkRadioButton(radio_frame, text=f"Camera {idx}{current_label}", variable=selected_camera,
                          value=idx, font=('Helvetica', 11)).pack(anchor="w", padx=10, pady=2)
        
        # === IP CAMERA SECTION ===
        ip_frame = ctk.CTkFrame(dialog)
        ip_frame.pack(pady=10, padx=20, fill="x")
        
        ctk.CTkRadioButton(ip_frame, text="IP Camera (RTSP)", variable=camera_type_var,
                      value=1, font=('Helvetica', 12, 'bold')).pack(anchor="w", padx=10, pady=5)
        
        # IP camera input fields
        input_frame = ctk.CTkFrame(ip_frame, fg_color="transparent")
        input_frame.pack(pady=5, padx=30, fill="x")
        
        # IP Address
        ctk.CTkLabel(input_frame, text="IP Address (e.g. 192.168.1.111):", anchor="w", font=('Helvetica', 10)).pack(fill="x", pady=(5,0))
        ip_var = ctk.StringVar(value="192.168.1.111")
        ctk.CTkEntry(input_frame, textvariable=ip_var, font=('Helvetica', 10)).pack(fill="x", pady=(0, 10))
        
        # RTSP Port
        ctk.CTkLabel(input_frame, text="RTSP Port (e.g. 554):", anchor="w", font=('Helvetica', 10)).pack(fill="x")
        port_var = ctk.StringVar(value="554")
        ctk.CTkEntry(input_frame, textvariable=port_var, font=('Helvetica', 10)).pack(fill="x", pady=(0, 10))
        
        # Username - REQUIRED
        ctk.CTkLabel(input_frame, text="Username (Required):", anchor="w", text_color="#f39c12", font=('Helvetica', 10, 'bold')).pack(fill="x")
        user_var = ctk.StringVar()
        user_entry = ctk.CTkEntry(input_frame, textvariable=user_var, font=('Helvetica', 10))
        user_entry.pack(fill="x", pady=(0, 10))
        
        # Password - REQUIRED
        ctk.CTkLabel(input_frame, text="Password (Required):", anchor="w", text_color="#f39c12", font=('Helvetica', 10, 'bold')).pack(fill="x")
        pass_var = ctk.StringVar()
        pass_entry = ctk.CTkEntry(input_frame, textvariable=pass_var, show="*", font=('Helvetica', 10))
        ...
        ctk.CTkButton(btn_frame, text="Switch", command=on_switch, fg_color="#27ae60",
                 font=('Helvetica', 11, 'bold'), width=100).pack(side="left", padx=5)
        ctk.CTkButton(btn_frame, text="Cancel", command=dialog.destroy, fg_color="#e74c3c",
                 font=('Helvetica', 11, 'bold'), width=100).pack(side="left", padx=5)
    ...
    def toggle_fugitive_mode(self):
        ...
        if not self.fugitive_mode:
            ...
            try:
                ...
                # Extract face encoding from fugitive image
                if self.insightface and self.insightface.app is not None:
                    # InsightFace uses BGR (already in self.fugitive_image)
                    face_locations = self.insightface.get_face_locations(self.fugitive_image)
                    if not face_locations:
                        messagebox.showerror("Error", "No face detected in selected image")
                        return
                    face_encodings = self.insightface.get_encodings(self.fugitive_image)
                else:
                    rgb_image = cv2.cvtColor(self.fugitive_image, cv2.COLOR_BGR2RGB)
                    face_locations = face_recognition.face_locations(rgb_image)
        ...
        if not self.is_running:
            try:
                ...
                if available_cameras:
                    for idx in available_cameras:
                        ctk.CTkRadioButton(radio_frame, text=f"Camera {idx}", variable=selected_camera,
                                      value=idx, font=('Helvetica', 11)).pack(anchor="w", padx=10, pady=2)
                else:
                    ctk.CTkLabel(radio_frame, text="No USB cameras detected", text_color="#e74c3c",
                ...
                ctk.CTkLabel(input_frame, text="Username (Required):", anchor="w", text_color="#f39c12", font=('Helvetica', 10, 'bold')).pack(fill="x")
                user_var = ctk.StringVar()
                ctk.CTkEntry(input_frame, textvariable=user_var, font=('Helvetica', 10)).pack(fill="x", pady=(0, 10))
                
                # Password - REQUIRED
                ctk.CTkLabel(input_frame, text="Password (Required):", anchor="w", text_color="#f39c12", font=('Helvetica', 10, 'bold')).pack(fill="x")
                pass_var = ctk.StringVar()
                ctk.CTkEntry(input_frame, textvariable=pass_var, show="*", font=('Helvetica', 10)).pack(fill="x", pady=(0, 10))
                ...
                ctk.CTkButton(btn_frame, text="Connect", command=on_select, fg_color="#27ae60",
                         font=('Helvetica', 11, 'bold'), width=100).pack(side="left", padx=5)
                ctk.CTkButton(btn_frame, text="Cancel", command=on_cancel, fg_color="#e74c3c",
                         font=('Helvetica', 11, 'bold'), width=100).pack(side="left", padx=5)
                ...
                if isinstance(self.camera_index, str) and self.camera_index.startswith("rtsp://"):
                    # ✅ IP Camera - use ThreadedIPCamera for smooth streaming
                    ...
                    # Wait for threaded grabber to get first frame
                    logger.warning("Waiting for IP camera stream...")
                    ...
                    # USB Camera - standard VideoCapture
                    self.is_ip_camera = False
                    self.cap = cv2.VideoCapture(self.camera_index)
                    if not self.cap.isOpened():
                        messagebox.showerror("Camera Error", f"Failed to open camera {self.camera_index}")
    ...
    def stop_camera(self):
        if self.is_running:
            ...
            # ✅ Stop threaded IP camera if using one
    ...
    def update_video_feed(self):
        ...
        # ✅ OPTIMIZATION: Update GUI labels only every 45 frames (~1.5 second) instead of every frame
        # This reduces GUI rendering overhead significantly
        # Note: frame_counter already incremented above for frame skipping logic
        
        if self.frame_counter % 45 == 0:  # Update labels every 45 frames
            current_time = time.time()
            elapsed = current_time - self.last_fps_time
            if elapsed > 0:
                self.current_fps = 45 / elapsed
            self.last_fps_time = current_time
            
            # ✅ PERFORMANCE: Periodic memory optimization
            if self.frame_counter % 180 == 0:  # Every 180 frames (~6 seconds at 30 FPS)
                optimize_memory()
        ...
        # Auto flush logs (only every 45 frames to reduce overhead)
        if self.frame_counter % 45 == 0:
        ...
        if self.video_label.winfo_exists():
            try:
                # ✅ FAST: Cache label dimensions - only recompute every 45 frames
                if self.frame_counter % 45 == 0 or not hasattr(self, '_cached_label_dims'):
    ...
    def process_tracking_frame_optimized(self, frame):
        ...
        # ==================== FUGITIVE MODE (Runs EVERY frame for instant detection) ====================
        # ✅ CRITICAL: Fugitive detection must be INSTANT - run every frame like guard tracking
        ...
        if self.is_fugitive_detection and self.fugitive_face_encoding is not None:
            ...
            try:
                ...
                if self.insightface and self.insightface.app is not None:
                    ...
                    fugitive_face_locations = self.insightface.get_face_locations(frame)
                    if fugitive_face_locations:
                        fugitive_face_encodings = self.insightface.get_encodings(frame)
                else:
                    ...
                    if fugitive_face_locations:
                        # ✅ PERFORMANCE: Use num_jitters=1 for speed
                ...
                if fugitive_face_locations:
                    for face_encoding, face_location in zip(fugitive_face_encodings, fugitive_face_locations):
                        # ✅ BALANCED: Tolerance similar to guard detection for consistency
                        if self.insightface and self.insightface.app is not None:
                             face_distance = self.insightface.compute_distance([self.fugitive_face_encoding], face_encoding)
                        else:
                             face_distance = face_recognition.face_distance([self.fugitive_face_encoding], face_encoding)
                        
                        confidence = 1.0 - face_distance[0]
                        
                        # ✅ BALANCED TOLERANCE: Same philosophy as guard - detect quickly but no false positives
                        ...
                        if is_fugitive_match:
                            ...
                            if self.targets_status and len(self.targets_status) > 0:
                                for guard_name, guard_status in self.targets_status.items():
                                    # ✅ MULTI-ANGLE: Check against ALL guard encodings, not just primary
                                    ...
                                    if guard_encodings:
                                        ...
                                        # ✅ STRICT: Guard must be SIGNIFICANTLY better (0.08 margin)
                                        ...
                                        # ✅ LARGER MARGIN: Skip if too close to any guard angle
        ...
        # - If all stable: 2-3 frames (67-100ms) - let tracker do the work
        ...
        if untracked_count > 0:
            ...
            # All visible and tracked - run detection LESS FREQUENTLY for better FPS
        ...
        # ✅ PERFORMANCE: Create rgb_full_frame here for detection
        rgb_full_frame = cv2.cvtColor(frame, cv2.COLOR_BGR2RGB)
        frame_h, frame_w = frame.shape[:2]
        
        # ✅ GPU ACCELERATION: Run YOLO Person Detection
        # This replaces/augments the "Ghost Detection" logic
        yolo_persons = []
        if self.yolo and self.yolo.context:
            try:
                yolo_persons = self.yolo.detect_persons(frame)
                # Draw YOLO boxes for debug
                # for (x1, y1, x2, y2) in yolo_persons:
            ...
            if success:
                ...
                if status["face_box"] is not None:
                    ...
                    if dx > movement_threshold or dy > movement_threshold or size_change > size_threshold:
                        ...
                        # ✅ DRIFT CHECK: Every 10 frames, verify tracker is following the correct person
                        if status["tracker_verify_counter"] >= 10:
                            status["needs_drift_verification"] = True
                            status["tracker_verify_counter"] = 0  # Reset counter
                            logger.debug(f"[DRIFT CHECK] {name}: Marking for identity verification")
                        
                        status["pending_tracker_box"] = new_box  # Store for later verification
                        
                        # Tracker still valid for now - apply smoothing
                        smoothed_box = smooth_bounding_box(new_box, status["face_box"], smoothing_factor=0.7)
                ...
                # ✅ CRITICAL: Force immediate re-detection on next frame
                self.re_detect_counter = 999  # Force detection to run immediately
                logger.warning(f"[TRACKER FAILED] {name}: Tracker returned False - FORCING immediate re-detection")
        ...
        # ✅ OPTIMIZED DETECTION - Run detection periodically to reduce lag
        # Only detect every Nth frame unless guards are missing
        self.re_detect_counter = getattr(self, 're_detect_counter', 0) + 1
        detection_interval = 8  # Detect every 8 frames for performance
        should_run_detection = (self.re_detect_counter >= detection_interval) or (len(untracked_targets) > 0 and self.re_detect_counter >= 3)
        ...
        if should_run_detection:
            ...
            # ✅ CRITICAL FIX: ONLY skip if ALL guards visible AND all confident
            # If ANY guard is missing, NEVER skip detection
            if all_guards_visible and all_trackers_confident and len(self.targets_status) > 0:
                ...
                # Use full resolution for better accuracy when guards are missing
                if len(untracked_targets) > 0:
                    # Guards missing - use GPU if available, else downscaled CPU detection
                    if self.insightface and self.insightface.app is not None:
                        face_locations = self.insightface.get_face_locations(frame)
                        logger.debug(f"[DETECT-GPU] {len(untracked_targets)} guards missing - using InsightFace GPU, found {len(face_locations)} faces")
                    else:
                        # CPU fallback - use 0.75x scale for balance between speed and accuracy
                        scale_factor = 0.75
                        h, w = rgb_full_frame.shape[:2]
                        scaled_w = max(1, int(w * scale_factor))
                        scaled_h = max(1, int(h * scale_factor))
                        rgb_scaled = cv2.resize(rgb_full_frame, (scaled_w, scaled_h), interpolation=cv2.INTER_LINEAR)
                        ...
                        logger.debug(f"[DETECT-CPU] {len(untracked_targets)} guards missing - using 0.75x downscale, found {len(face_locations)} faces")
                else:
                    # All guards tracked but low confidence - use downscaled for speed
                    if self.insightface and self.insightface.app is not None:
                        # InsightFace is fast enough on GPU, no need to downscale usually
                        # But if we want to match logic:
                        face_locations = self.insightface.get_face_locations(frame)
                    else:
                        # DISABLED: Always use full resolution for maximum accuracy
                        # scale_factor = 2.0
                        # h, w = rgb_full_frame.shape[:2]
                        # scaled_w = max(1, int(w / scale_factor))
                        # scaled_h = max(1, int(h / scale_factor))
            ...
            any_tracker_needs_verify = any(
                status.get("tracker_verify_counter", 0) >= 14  # Will trigger verify on next increment
                for status in self.targets_status.values()
                if status.get("visible", False)
            )
            
            if face_locations and (len(untracked_targets) > 0 or has_potential_unknown or any_tracker_needs_verify):
                # Encode faces for matching OR drift detection - use GPU if available
                if self.insightface and self.insightface.app is not None:
                    face_encodings = self.insightface.get_encodings(frame)
                    logger.debug(f"[ENCODE-GPU] Encoded {len(face_locations)} faces with InsightFace GPU")
                else:
                    # CPU encoding with num_jitters=1 for speed (was default 1)
                    face_encodings = face_recognition.face_encodings(rgb_full_frame, face_locations, num_jitters=1)
                    logger.debug(f"[ENCODE-CPU] Encoded {len(face_locations)} faces (num_jitters=1)")
                ...
                # All guards tracked, no unknown persons, no verify needed - skip encoding
                face_encodings = []
            else:
                face_encodings = []
            ...
            for name, status in self.targets_status.items():
                ...
                if tracker_conf > 0.95:
                    # Tracker is very stable - assume face is still at tracked location
                ...
                if face_locations and face_encodings and len(face_encodings) > 0:
                    ...
                    if all_guard_encodings:
                        ...
                        for face_idx, unknown_enc in enumerate(face_encodings):
                            ...
                            if self.insightface and self.insightface.app is not None:
                                primary_dist = self.insightface.compute_distance([primary_enc], unknown_enc)[0]
                            else:
                            ...
                            if primary_dist < 0.50:
                                ...
                                if self.insightface and self.insightface.app is not None:
                                    distances = self.insightface.compute_distance(all_guard_encodings, unknown_enc)
                                else:
                            ...
                            # ✅ BALANCED: Accept if distance < 0.58 (relaxed for better detection at angles)
                            if best_dist < 0.58:
                                face_found_at_tracked_location = True
                ...
                if not face_found_at_tracked_location:
                    ...
                    # If tracker is tracking a face but NO pose is detected for 10+ frames, it's a GHOST
                    # ✅ GPU ENHANCEMENT: Also check YOLO person detection
                    
                    has_yolo_person = False
                    if yolo_persons:
                        # Check if tracker box overlaps with any YOLO person
                        tx1, ty1, tx2, ty2 = status.get("face_box") if status.get("face_box") else (0,0,0,0)
                        if status.get("face_box"):
                    ...
                    # If we have YOLO, we trust it. If no YOLO person overlaps, it's likely a ghost.
                    # But YOLO might miss small/far people. So we combine with Pose.
                    
                    # ✅ GHOST DETECTION REFINED:
                    # 1. "Chair/Object" Ghost: No Pose AND No YOLO Person -> Kill FAST (30 frames / 1s)
                    # 2. "Static Person" Ghost: No Pose but YOLO Person exists -> Keep longer (450 frames / 15s)
                    
                    is_likely_object = (pose_confidence < 0.20 and skeleton_keypoints < 5 and (not has_yolo_person if self.yolo else True))
                    
                    if is_likely_object:
                        # No valid pose detected AND (if YOLO available) no person detected
                        # Increment ghost counter
                        status["ghost_no_pose_frames"] = status.get("ghost_no_pose_frames", 0) + 1
                        
                        # Determine threshold based on whether we have ANY evidence of a person
                        # If YOLO says "person", we give benefit of doubt (15s). If nothing says person, kill fast (1s).
                        ghost_threshold = 450 if has_yolo_person else 30
                        
                        if status["ghost_no_pose_frames"] >= ghost_threshold:
                            # ✅ GHOST CONFIRMED
                            logger.warning(f"[GHOST - NO POSE/PERSON] {name}: Face tracked but NO POSE/PERSON for {status['ghost_no_pose_frames']} frames ({ghost_threshold/30:.1f}s) - HARD RESET")
                            ...
                            logger.debug(f"[GHOST CHECK] {name}: No pose/person frame {status['ghost_no_pose_frames']}/{ghost_threshold} (pose={pose_confidence:.2f}, keypts={skeleton_keypoints}, yolo={has_yolo_person})")
                    else:
                        # Pose detected OR YOLO person detected - reset ghost counter
                        status["ghost_no_pose_frames"] = 0
                    
                    # ✅ NEW REQUIREMENT: Face-Only Hard Reset (5s Rule)
                    # If we have a verified face (high confidence) but NO valid skeleton for > 5 seconds,
                    # force a hard reset to re-acquire the full body properly.
                    # This handles cases where the tracker is stuck on a face but the body/skeleton is wrong or missing.
                    
                    face_conf = status.get("face_confidence", 0.0)
                    pose_conf = status.get("pose_confidence", 0.0)
                    
                    if face_conf > 0.60 and pose_conf < 0.30:
                        # We have a face, but poor pose
                        status["face_only_no_body_frames"] = status.get("face_only_no_body_frames", 0) + 1
                        
                        if status["face_only_no_body_frames"] >= 150: # 5 seconds at 30fps
                             logger.warning(f"[FACE-ONLY RESET] {name}: Face verified ({face_conf:.2f}) but NO SKELETON for 5s - HARD RESET to re-acquire body")
                             
                             # Hard reset to force full re-detection
                             status["visible"] = False
                             status["tracker"] = None
                             status["body_tracker"] = None
                             status["face_box"] = None
                             ...
                             status["missing_pose_counter"] = 0
                             status["ghost_no_pose_frames"] = 0
                             status["face_only_no_body_frames"] = 0
                             status["missing_start_time"] = None
                             self.re_detect_counter = 999
                             continue
                    else:
                        # Reset counter if we have good pose or lost face confidence
                        status["face_only_no_body_frames"] = 0
                    ...
                    if body_box_valid and pose_quality_valid and keypoint_valid and body_in_frame:
                        ...
                        if face_in_body_area or pose_confidence > 0.60:
                            ...
                            status["missing_pose_counter"] = 0
                            status["ghost_no_pose_frames"] = 0  # Reset ghost counter
                            status["missing_start_time"] = None  # Reset timer for fresh detection
                            self.re_detect_counter = 999  # Force immediate re-detection
            ...
            if face_locations and len(face_locations) > 0 and len(untracked_targets) > 0 and face_encodings:
                ...
                # ✅ OPTIMIZED VALUES: Better guard detection with reasonable false positive protection
                if brightness < 100:  # Low-light mode
                    adaptive_params = {
                        "brightness": int(brightness),
                        "tolerance": 0.58,  # Relaxed for dark to catch guards
                        "confidence": 0.42  # Lower threshold for better detection
                    }
                    logger.debug(f"Dark mode activated: brightness={brightness:.0f}")
                else:
                    adaptive_params = {
                        "brightness": int(brightness),
                        "tolerance": 0.60,  # More relaxed for normal lighting
                        "confidence": 0.40  # Lower threshold for better detection
                    }
                ...
                # ✅ OPTIMIZED TOLERANCE: Better guard detection with reasonable protection
                # Priority: Detect guards reliably while minimizing false positives
                if adaptive_params["brightness"] < 100:  # Low-light mode
                    base_tolerance = 0.56  # Relaxed for dark conditions
                    min_confidence = 0.42  # Lower to detect in poor lighting
                    logger.debug(f"Using LOW-LIGHT tolerances: tolerance={base_tolerance:.2f}, min_confidence={min_confidence:.2f}")
                elif num_guards >= 2:
                    # MULTI-GUARD MODE: Moderate strictness to prevent cross-matching
                    base_tolerance = 0.54  # Balanced for multi-guard
                    min_confidence = 0.44  # Moderate threshold
                else:
                    # SINGLE GUARD: More relaxed for fast ID
                    base_tolerance = 0.58  # Relaxed tolerance
                    min_confidence = 0.40  # Lower threshold for better detection
                ...
                for face_idx, face_location in enumerate(face_locations):
                    ...
                    # ✅ OPTIMIZED VALIDATION: More permissive for better detection
                    ...
                    MIN_FACE_SIZE = 15  # Reduced from 20 for better distance detection
                    ASPECT_RATIO_MIN = 0.25  # Very permissive for extreme upward angle
                    ASPECT_RATIO_MAX = 3.0  # Very permissive for extreme angles
                ...
                for target_idx, name in enumerate(untracked_targets):
                    ...
                    for face_idx, face_width, face_height in valid_face_indices:
                        ...
                        primary_enc = all_guard_encodings[0]
                        if self.insightface and self.insightface.app is not None:
                            primary_dist = self.insightface.compute_distance([primary_enc], unknown_encoding)[0]
                        else:
                        ...
                        if primary_dist < 0.50 or len(all_guard_encodings) == 1:
                            ...
                            if self.insightface and self.insightface.app is not None:
                                all_distances = self.insightface.compute_distance(all_guard_encodings, unknown_encoding)
                            else:
                        ...
                        if self.is_fugitive_detection and self.fugitive_face_encoding is not None:
                            if self.insightface and self.insightface.app is not None:
                                fugitive_dist = self.insightface.compute_distance([self.fugitive_face_encoding], unknown_encoding)[0]
                            else:
                        ...
                        if best_dist <= strict_tolerance:
                            ...
                            if best_dist > 0.45:  # Higher distance - log it
                                logger.debug(f"[ANGLE MATCH] {name} vs face_{face_idx}: Distance={best_dist:.3f} (angle {best_angle_idx+1})")
                        else:
                            logger.debug(f"[SKIP] {name} vs face_{face_idx}: Distance={best_dist:.3f} exceeds strict tolerance={strict_tolerance:.2f}")
                ...
                for item in cost_matrix:
                    ...
                    if confidence >= min_confidence:
                        ...
                        if status.get("visible", False) and status.get("stable_tracking", False):
                            ...
                            # ✅ INSTANT: Guard not visible - assign IMMEDIATELY (no confirmation needed)
                            status["pending_face_idx"] = None
                            status["pending_confirm_count"] = 0
        ...
        for name, status in self.targets_status.items():
            if status.get("needs_drift_verification", False) and status.get("visible", False):
                ...
                # ✅ FIX: Handle case where NO faces are found near tracker
                # If tracker is active but face detector sees NOTHING near it, it might be tracking a chair/object
                faces_near_tracker_count = 0
                ...
                for face_idx, (top, right, bottom, left) in enumerate(face_locations):
                    ...
                    if distance_to_tracker < 100:
                        ...
                        if face_idx < len(face_encodings) and face_encodings[face_idx] is not None:
                            ...
                            if best_dist < 0.45:  # Strict tolerance for drift check
                                guard_found_near_tracker = True
                                # Reset drift counter on successful verification
                                status["tracker_drift_counter"] = 0
                                status["no_face_near_tracker_counter"] = 0 # Reset no-face counter
                ...
                # ✅ FIX: Handle "No Face Found" case (Tracker on chair/object)
                if faces_near_tracker_count == 0:
                    # Tracker is active, but face detector sees NO faces nearby
                    # This strongly suggests tracker is on a non-face object (chair, bag, etc.)
                    status["no_face_near_tracker_counter"] = status.get("no_face_near_tracker_counter", 0) + 1
                    
                    if status["no_face_near_tracker_counter"] >= 5: # 5 checks = 50 frames = ~1.5s
                         logger.warning(f"[DRIFT DETECTED] {name}: Tracker active but NO FACES found nearby for 5 checks - likely tracking object")
                         # Treat as drift
                         unknown_face_near_tracker = True
                else:
                    status["no_face_near_tracker_counter"] = 0
                ...
                if unknown_face_near_tracker and not guard_found_near_tracker:
                    ...
                    logger.warning(f"[DRIFT DETECTED] {name}: Tracker may be following unknown person/object (drift_count={drift_count}/3)")
                    ...
                    if drift_count >= 3:
                        ...
                        status["missing_start_time"] = None
                        self.re_detect_counter = 999  # Force immediate re-detection
                        logger.warning(f"[DRIFT RESET] {name}: Tracker drifted to unknown person/object — HARD RESET triggered")
        
        # ✅ HARD RESET: If a guard has been missing for > 5 seconds, clear all tracking state
        # This prevents ghost tracking and forces a clean re-identification flow
        # OPTIMIZED: Reduced from 30s to 5s for faster re-detection
        for name, status in self.targets_status.items():
            if not status.get("visible", False):
                if status.get("missing_start_time") is None:
                    ...
                    if missing_duration >= 5.0 and not status.get("hard_reset_done", False):
                        try:
                            status["tracker"] = None
                            status["body_tracker"] = None
                            status["face_box"] = None
