            ...
            if self.frame_counter % 3 == 0:
                ...
                try:
                    ...
                    if face_locations:
                        ...
                        angle_aware_tolerance = 0.65  # STRICTER: Prevent false fugitive alerts (was 0.72)
                        min_confidence_threshold = 0.32  # STRICTER: Better confidence required (was 0.25)
                        
                        is_fugitive_match = (face_distance[0] <= angle_aware_tolerance and 
                                           confidence >= min_confidence_threshold)
                        
                        if is_fugitive_match:
                            # ✅ CRITICAL FIX: STRICT SEPARATION - NEVER confuse guard as fugitive
                            # Cross-check: Make sure this isn't a guard that's being misidentified as fugitive
                            is_likely_guard = False
                            margin = getattr(self, 'guard_fugitive_margin', 0.12)
                            ...
                            if self.targets_status and len(self.targets_status) > 0:
                                ...
                                for guard_name, guard_status in self.targets_status.items():
                                    ...
                                    if guard_encoding is not None:
                                        ...
                                        # ✅ STRICT RULE 1: If guard matches better - it's a guard, NOT fugitive
                                        if guard_dist < face_distance[0]:
                                            logger.warning(f"[STRICT SKIP FUGITIVE] Face is GUARD '{guard_name}' (guard_dist={guard_dist:.3f} < fugitive_dist={face_distance[0]:.3f})")
                                            is_likely_guard = True
                                            break
                                        
                                        # ✅ STRICT RULE 2: If within margin of guard - too risky, skip
                                        if abs(guard_dist - face_distance[0]) < margin:
                                            logger.warning(f"[STRICT SKIP FUGITIVE] Face TOO CLOSE to guard '{guard_name}' (guard={guard_dist:.3f}, fugitive={face_distance[0]:.3f}, margin={margin:.2f})")
                                            is_likely_guard = True
                                            break
        ...
        if should_run_detection:
            ...
            if face_locations and len(face_locations) > 0:
                ...
                if len(untracked_targets) == 0 or not face_encodings:
                    ...
                    # ✅ BALANCED: Tolerance values balanced for stability AND angle detection
                    ...
                    for target_idx, name in enumerate(untracked_targets):
                        ...
                        for face_idx, face_width, face_height in valid_face_indices:
                            ...
                            if self.is_fugitive_detection and self.fugitive_face_encoding is not None:
                                ...
                                # ✅ STRICT SEPARATION: Require significant margin to identify as guard
                                # If face matches fugitive within margin, REJECT guard match completely
                                margin = getattr(self, 'guard_fugitive_margin', 0.12)
                                
                                # Case 1: Face matches fugitive better - REJECT
                                if fugitive_dist < dist:
                                    logger.warning(f"[STRICT REJECT] {name} vs face_{face_idx}: Face is FUGITIVE (fugitive_dist={fugitive_dist:.3f} < guard_dist={dist:.3f})")
                                    continue
                                
                                # Case 2: Face matches both similarly (within margin) - REJECT (too risky)
                                if abs(dist - fugitive_dist) < margin:
                                    logger.warning(f"[STRICT REJECT] {name} vs face_{face_idx}: TOO CLOSE to fugitive (guard={dist:.3f}, fugitive={fugitive_dist:.3f}, margin={margin:.2f})")
                                    continue
                                
                                # Case 3: Guard matches significantly better - OK to proceed
                                logger.debug(f"[GUARD OK] {name} vs face_{face_idx}: Guard confirmed (guard={dist:.3f}, fugitive={fugitive_dist:.3f}, gap={fugitive_dist-dist:.3f})")
                    ...
                    for item in cost_matrix:
                        ...
                        if confidence >= min_confidence:
                            ...
                            if status.get("visible", False) and status.get("stable_tracking", False):
                                # Guard already tracked - check if this is consistent match
                                if pending_face_idx == face_idx:
                                    ...
                                    if status["pending_confirm_count"] < confirm_threshold:
                                        ...
                                        # Don't reassign yet - keep current tracker, wait for confirmation
                                        continue
                                    else:
                                ...
                                # Guard not visible or not stable - assign immediately (no confirmation needed)
                                status["pending_face_idx"] = None
                                status["pending_confirm_count"] = 0
