    ...
    try:
        ...
        return {
            "detection": {"min_detection_confidence": 0.5, "min_tracking_confidence": 0.5, 
                         "face_recognition_tolerance": 0.52, "re_detect_interval": 5},  # ✅ BALANCED: 0.52 tolerance to prevent ghost, 5 frame interval
            ...
            if success:
                ...
                if status["face_box"] is not None:
                    ...
                    if dx > movement_threshold or dy > movement_threshold or size_change > size_threshold:
                        ...
                        # ✅ DRIFT CHECK: Every 15 frames, verify tracker is following the correct person
                        if status["tracker_verify_counter"] >= 15:
        ...
        if should_run_detection:
            ...
            if all_guards_visible and all_trackers_confident and len(self.targets_status) > 0:
                ...
                if len(untracked_targets) > 0:
                    # Guards missing - use FULL RESOLUTION for best accuracy
                    face_locations = face_recognition.face_locations(rgb_full_frame, model="hog")
                    logger.debug(f"[DETECT] {len(untracked_targets)} guards missing - using FULL resolution, found {len(face_locations)} faces")
                else:
            ...
            if face_locations and (len(untracked_targets) > 0 or has_potential_unknown or any_tracker_needs_verify):
                # Encode faces for matching OR drift detection
            ...
            for name, status in self.targets_status.items():
                ...
                if not face_found_at_tracked_location:
                    ...
                    if pose_confidence < 0.20 and skeleton_keypoints < 5:
                        ...
                        if status["ghost_no_pose_frames"] >= 10:
                            # ✅ GHOST CONFIRMED: Face tracked but NO POSE for 10+ frames
                            # This is definitely a ghost (bag, poster, shadow, wrong person's face)
                            logger.warning(f"[GHOST - NO POSE] {name}: Face tracked but NO POSE for {status['ghost_no_pose_frames']} frames - HARD RESET")
                            ...
                            logger.debug(f"[GHOST CHECK] {name}: No pose frame {status['ghost_no_pose_frames']}/10 (pose={pose_confidence:.2f}, keypts={skeleton_keypoints})")
                    else:
