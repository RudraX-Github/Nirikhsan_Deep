                    ...
                    for face_encoding, face_location in zip(face_encodings, face_locations):
                        ...
                        if is_fugitive_match:
                            ...
                            if not getattr(self, 'fugitive_currently_visible', False):
                                ...
                                # 2. SNAPSHOT & LOG ENTRY: Immediate on appearance
                                try:
                                    # Capture snapshot immediately
                                    snapshot_path = self.capture_alert_snapshot(frame, f"FUGITIVE_{self.fugitive_name}_APPEARED", check_rate_limit=False)
                                    img_path = snapshot_path if snapshot_path else "N/A"
                                    
                                    # Create CSV log entry for fugitive appearance
                                    self.temp_log.append((
                                        time.strftime("%Y-%m-%d %H:%M:%S"),
                                        f"FUGITIVE_{self.fugitive_name}",
                                        "FUGITIVE_DETECTED",
                                        f"APPEARANCE (confidence: {confidence:.3f})",
                                        img_path,
                                        f"{confidence:.3f}"
                                    ))
                                    ...
                                    logger.warning(f"[FUGITIVE DETECTED] ⚠️ {self.fugitive_name} APPEARED in camera - Snapshot captured, alert triggered for 15s (confidence: {confidence:.3f})")
                                except Exception as e:
                                    logger.error(f"[FUGITIVE LOG] Error creating fugitive appearance log: {e}")
                            else:
