        ...
        if should_run_detection:
            ...
            if face_locations and len(face_locations) > 0:
                ...
                if len(untracked_targets) == 0 or not face_encodings:
                    ...
                    # ✅ UNKNOWN PERSON DETECTION - VALIDATION ONLY (No logging/snapshots)
                    # This check exists purely to distinguish between unmatched faces and actual guards/fugitives
                    # during internal processing. Do not log or create snapshots for unidentified faces.
                    for face_idx in range(len(face_locations)):
                        if face_idx not in assigned_faces:
                            # This face doesn't match any guard - validate it's not a fugitive
                            if face_idx >= len(valid_face_indices):
                                continue
                            
                            is_unknown = True
                            
                            # ✅ CRITICAL: Cross-check - don't consider fugitive as unknown
                            if self.is_fugitive_detection and self.fugitive_face_encoding is not None:
                                if face_idx < len(face_encodings):
                            ...
                            # ✅ INTERNAL VALIDATION ONLY - No logging or snapshot creation
                            # Unknown person concept exists only to prevent confusion during processing
