...
class PoseApp:
    ...
    def snap_photo(self):
        ...
        if self.onboarding_step == 0:
            ...
            # ✅ SPECIFICATION: 4 poses captured during onboarding (can detect 7 actions in real-time)
    ...
    def process_tracking_frame_optimized(self, frame):
            ...
            try:
                ...
                if face_locations:
                    ...
                    for face_encoding, face_location in zip(face_encodings, face_locations):
                        ...
                        # ✅ RELAXED TOLERANCE: Use 0.68 for instant detection (same as guards)
                        ...
                        if match[0]:  # If face matches - FUGITIVE FOUND!
                            # ✅ CRITICAL: Also check confidence threshold for fugitive
                            # Don't trust extremely low confidence matches even if within tolerance
                            confidence = 1.0 - face_distance[0]
                            if confidence < 0.30:  # Reject matches with confidence < 0.30 (distance > 0.70)
                                logger.debug(f"[SKIP FUGITIVE] Face match too weak - confidence={confidence:.3f}, distance={face_distance[0]:.3f}")
                                continue
                            ...
                            # ✅ CRITICAL: Always trigger sound and logging EVERY TIME detected
                            ...
                            try:
                                ...
                                if not hasattr(self, 'fugitive_alert_sound_thread') or \
                                   ...
                                   not self.fugitive_alert_sound_thread.is_alive():
                                    ...
                                    logger.warning(f"[FUGITIVE ALERT] ⚠️ FUGITIVE DETECTED INSTANTLY - {self.fugitive_name} - Playing 30s alert!")
                            except Exception as e:
        ...
        if untracked_count > 0:
            ...
            adaptive_interval = 1  # 33ms - EVERY FRAME until all targets re-acquired
        ...
        if should_run_detection:
                                    ...
                                    for tracker_type in tracker_types:
                                        try:
                                            if tracker_type == 'CSRT':
                                                ...
                                                new_tracker = cv2.legacy.TrackerMedianFlow_create()
                                            ...
                                            if new_tracker is not None:
                                                ...
                                                status["back_view_tracking"] = True
                                                status["skeleton_track_start_frame"] = self.frame_counter
                                                logger.warning(f"[BACK VIEW] {name}: Created fresh {tracker_type} tracker with skeleton bbox "
                                                             f"(face missing {face_missing_frames} frames, pose: {pose_confidence:.2f})")
                                                break
                                        except Exception as e:
                                            logger.debug(f"Failed {tracker_type} init: {e}")
                                            new_tracker = None
                                            continue
                            except Exception as e:
            ...
            if face_locations and len(face_locations) > 0:
                ...
                if len(untracked_targets) == 0 or not face_encodings:
                    ...
                    # ✅ OPTIMIZED: Use adaptive thresholds based on lighting conditions
                    num_guards = len(untracked_targets)
                    
                    # ✅ CRITICAL FIX: STRICTER matching thresholds to prevent wrong guard assignment
                    # Previous tolerances (0.65-0.68) were TOO relaxed, causing wrong guard matching
                    # New approach: Use BEST-MATCH-ONLY strategy - require high confidence minimum
                    if adaptive_params["brightness"] < 100:  # Low-light mode
                        base_tolerance = 0.50  # TIGHTER: Guard must match reasonably well even in dark
                        min_confidence = 0.50  # TIGHTER: Minimum 50% confidence to avoid false matches
                        logger.debug(f"Using TIGHT low-light tolerances for accuracy: tolerance={base_tolerance:.2f}, min_confidence={min_confidence:.2f}")
                    elif num_guards >= 2:
                        # MULTI-GUARD MODE: STRICTER to avoid cross-matching between guards
                        base_tolerance = 0.50  # TIGHTER: Each face must clearly match ONE guard
                        min_confidence = 0.50  # TIGHTER: Minimum 50% confidence
                    else:
                        # SINGLE GUARD: STILL STRICTER to avoid matching wrong face
                        base_tolerance = 0.52  # TIGHTER: Only clear matches
                        min_confidence = 0.48  # TIGHTER: Minimum 48% confidence
                ...
                if bx1 < bx2 and by1 < by2:
                    ...
                    if crop.size != 0:
                        ...
                        if results_crop is not None and results_crop.pose_landmarks is not None:
                            ...
                            # ✅ NEW PIPELINE: Simplified for MoveNet format
